...
interface CutlistTableProps {
  ...
  onSendWhatsApp?: (phoneNumber: string, data: CutlistData) => void;
}

const CutlistTable: React.FC<CutlistTableProps> = ({ initialData, onSave, onSendWhatsApp }) => {
  ...
  const handleSendWhatsApp = () => {
    ...
    if (onSendWhatsApp) {
      onSendWhatsApp(phoneNumber, data);
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <TabPanel value={tabValue} index={0}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
          <Typography variant="h6">Stock Pieces ({data.unit})</Typography>
          <Button 
            variant="contained" 
            startIcon={<AddIcon />}
            ...
            <TableBody>
              {data.stockPieces.map((piece) => (
                <TableRow key={piece.id}>
                  ...
                  <TableCell>
                    <IconButton onClick={() => handleDeleteStockPiece(piece.id)} color="error">
                      <DeleteIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
      ...
      <TabPanel value={tabValue} index={1}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
          <Typography variant="h6">Cut Pieces ({data.unit})</Typography>
          <Button 
            variant="contained" 
            startIcon={<AddIcon />}
            ...
            <TableBody>
              {data.cutPieces.map((piece) => (
                <TableRow key={piece.id}>
                  ...
                  <TableCell>
                    <IconButton onClick={() => handleDeleteCutPiece(piece.id)} color="error">
                      <DeleteIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
